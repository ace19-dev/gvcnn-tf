...
class Data(object):
    ...
    def _parse_function(self, start, end, selected):
        images = []
        labels = []
        batch_img = self.images[selected][start:end]
        ...
        labels.extend(self.labels[selected][start:end])
    ...
    def next_batch(self, batch_size):
        ...
        if end > self.data_size(selected):
            end = self.data_size(selected) + 1

        self._shuffle(selected)
        self.cls_start_step[selected] = 0

        return self._parse_function(start, end, selected)
